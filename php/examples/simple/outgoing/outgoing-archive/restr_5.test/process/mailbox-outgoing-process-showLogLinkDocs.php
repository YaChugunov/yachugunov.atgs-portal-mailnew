...
$_reqDB = mysqli_fetch_assoc(mysqlQuery("SELECT inbox_rowIDadd_rel FROM " . __MAIL_OUTGOING_TABLENAME . " WHERE koddocmail='{$koddocmail}'"));
$rowIDS = $_reqDB['inbox_rowIDadd_rel'] != "" && $_reqDB['inbox_rowIDadd_rel'] != NULL ? $_reqDB['inbox_rowIDadd_rel'] : "-";
#
#
/*
...
Editor::inst($db, __MAIL_INCOMING_TABLENAME)
	...
	->on('preGet', function ($editor, $id) use ($rowIDS) {
		$editor->where(function ($q) use ($rowIDS) {
			...
			$q->order(__MAIL_INCOMING_TABLENAME . '.inbox_docDate desc');
		});
	})
