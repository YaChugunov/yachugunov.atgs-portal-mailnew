...
if (isset($_SESSION['password']) && isset($_SESSION['login'])) {
	if (checkUserAuthorization($_SESSION['login'], $_SESSION['password']) == -1) {
		...
		if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && $_koddocmail != "" && $_parameter != "") {
			$_reqDB = mysqli_fetch_array(mysqlQuery("SELECT * FROM " . __MAIL_OUTGOING_PREFIX . "_logChanges WHERE `action` IN ('CRT','UPD') AND `koddocmail` = '{$_koddocmail}' AND `koddel` <> 'deleted' AND `changes` LIKE '%{$_parameter}%' ORDER BY `timestamp` DESC LIMIT 1"));
