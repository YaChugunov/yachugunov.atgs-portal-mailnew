...
function clearRelFields($db, $id, $values) {
	#
	$_REQ_1 = $db->sql("SELECT inbox_rowID_rel FROM " . __MAIL_OUTGOING_TABLENAME . " WHERE id=" . $id)->fetchAll();
	...
	if (empty($inbox_rowID_relV) && !empty($inbox_rowID_rel)) {
		...
		$docTYPEprev = $_REQ_2[0]['inbox_docType_prev'];
...
function updateFields($db, $action, $id, $values, $row) {
	...
	if ($action == 'CRT') {
		...
		if (!empty($inbox_rowID_rel) and $outbox_docType >= 2) {
			...
			if (isset($_REQ_Incoming) && !empty($_REQ_Incoming)) {
				...
				if ($_REQ_Incoming[0]['inbox_docType'] == 3) {
					$db->update(__MAIL_INCOMING_TABLENAME, array(
	...
	if (!empty($inbox_rowID_rel)) {
		...
		# 
		$_REQ_Incoming = $db->sql("SELECT inbox_docType, inbox_docType_prev, inbox_docID, inbox_UID, koddocmail, inbox_docFileID FROM " . __MAIL_INCOMING_TABLENAME . " WHERE id=" . $inbox_rowID_rel)->fetchAll();
		#
		if ($outbox_docType == 2) {
			#
			if (isset($_REQ_Incoming) && !empty($_REQ_Incoming)) {
...
function sendEmailToIspol_FullCheckout($db, $action, $id, $values, $row) {
	...
	// <<<<< ОТЛАДКА
	//
	if ($__controlCheckout == "1" && ($_sqlReq_1[0]['outbox_controlIspolMailNotifyCheckout'] == "1" or $values[__MAIL_OUTGOING_TABLENAME]['outbox_controlIspolMailNotifyCheckout'] == "1")) {
		$arrispol = explode(",", $__contrKOD);
		$arrFileIDAdd = explode(",", $__docFileIDadd);
		foreach ($arrispol as $value) {
			$__contractorDATA1 = $db->sql("SELECT id, emailaddress, namezayvfio FROM mailbox_sp_users WHERE kodispolout=" . $value)->fetchAll();
...
function updateIspolCheckout($db, $id, $values, $row) {
	...
	if (($cntIspol == $cntCheckouts) && ($row[__MAIL_OUTGOING_TABLENAME]['outbox_controlIspolCheckout'] != "1")) {
...
function newUserMessage($db, $action, $id, $values) {
	...
	if ($_SESSION['outbox_checkChanges_is'] == "changed") {
		if ((!empty($_QRY[0]['outbox_docContractorID']) && $_QRY[0]['outbox_docContractorID'] != "1") && (!empty($_QRY[0]['outbox_docContractor_kodispolout']) && $_QRY[0]['outbox_docContractor_kodispolout'] != "000000000000000")) {
			$msg_maintext = '';
			$msg_subtext1 = '';
			$msg_subtext2 = '';
			$msg_subtext3 = '';
			...
			if ($_QRY[0]['outbox_docContractorMULTI'] == "1") {
				$for_singleuser = "";
			...
			$msg_id_new = $newKod;
			$msg_title = ($action == "CRT") ? "Новый исходящий документ" : "Изменения по исходящему документу";
