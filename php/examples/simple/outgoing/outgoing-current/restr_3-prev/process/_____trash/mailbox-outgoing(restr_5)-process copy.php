		...
		if (!$noChanges) {
		  ...
		  */
				if ($key == 'outbox_docDate') {
					...
					$reqDB_old = $db->sql("SELECT namezakshot FROM sp_contragents WHERE kodzakaz='" . $oldSettings[$key] . "'")->fetchAll();
					$reqDB_new = $db->sql("SELECT namezakshot FROM sp_contragents WHERE kodzakaz='" . $value . "'")->fetchAll();
					$oldVal = $reqDB_old[0]['namezakshot'];
					$newVal = $reqDB_new[0]['namezakshot'];
...
function updateFields($db, $action, $id, $values) {
	...
	if (json_encode($values['enblRecipientManual'], JSON_NUMERIC_CHECK) != '[1]') {
		$__zakazDATA = $db->sql("SELECT kodzakaz, namezakshot, zakfio FROM sp_contragents WHERE kodzakaz=" . $__zakazKOD[0]['outbox_docRecipient_kodzakaz'])->fetchAll();
		if ($__zakazKOD[0]['outbox_docRecipient_kodzakaz'] != '000000000000000') {
			$zakaz = $__zakazDATA[0]['namezakshot'];
	...
	if ($action == 'CRT') {
		...
		$db->update('mailbox_outgoing_test', array(
			// 			'outbox_docDate'			=> date('Y-m-d H:i:s'),
			'koddocmail'				=> $__newKod,
		...
		if ($__SQL1) {
			...
			if ($__relID <> '0' and !empty($__relID) and $__SQL1[0]['outbox_docType'] >= 2) {
