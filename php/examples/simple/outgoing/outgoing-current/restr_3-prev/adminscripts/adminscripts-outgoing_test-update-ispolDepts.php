...
if (isset($_SESSION['password']) && isset($_SESSION['login'])) {
	if (checkUserAuthorization($_SESSION['login'], $_SESSION['password']) == -1) {
		...
		if ($_SESSION['id'] == '999') {
			$_req1 = mysqlQuery("SELECT koddocmail, outbox_docContractor_kodispolout FROM mailbox_outgoing_test WHERE outbox_docDate BETWEEN '2021-01-01 00:00:00' AND '2022-12-31 23:59:59'");
			while ($_req1Rows = mysqli_fetch_assoc($_req1)) {
				...
				$_req3 = mysqlQuery("UPDATE mailbox_outgoing_test SET outbox_docContractorDEPT='{$ispoldepts}' WHERE koddocmail='{$koddocmail}'");
				$output = "Ok";
			}
		} else {
			$output = 'Error -2';
