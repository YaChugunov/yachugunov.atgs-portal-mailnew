<?php
if (isset($_GET['uid'])) {
    $_reqDB_docmail = mysqli_fetch_assoc(mysqlQuery("SELECT * FROM " . __MAIL_OUTGOING_TABLENAME . " WHERE koddocmail='{$_GET['uid']}'"));
}
if (isset($_GET['type']) && !empty($_GET['type'])) {
    if ($_GET['type'] == 'out' && isset($_GET['mode']) && $_GET['mode'] == "profile") {
        $noerrors = true;
?>
        <div id="profile-title" class="pt-2 text-center">
            <div class="p-3 mb-2 bg-light text-dark shadow">
                ...
                <h3 class="mb-0 text-center text-dark">
                    <?php echo $noerrors ? "Исходящая почта № 1-1/" . $_reqDB_docmail['outbox_docIDSTR'] : ""; ?></h3>
