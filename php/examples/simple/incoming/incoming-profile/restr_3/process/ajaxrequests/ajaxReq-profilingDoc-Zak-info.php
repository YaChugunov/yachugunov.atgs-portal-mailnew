...
if (isset($_SESSION['password']) && isset($_SESSION['login'])) {
    if (checkUserAuthorization($_SESSION['login'], $_SESSION['password']) == -1) {
        ...
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && $_koddocmail != "") {
            $_QRY_Main = mysqli_fetch_assoc(mysqlQuery("SELECT inbox_docIDSTR, inbox_docSender_kodzakaz, inbox_docSourceID, inbox_docSourceDate FROM " . __MAIL_INCOMING_TABLENAME . " WHERE koddocmail='{$_koddocmail}'"));
            $kodzakaz = $_QRY_Main['inbox_docSender_kodzakaz'];
            $_QRY_ContrAgent = mysqli_fetch_assoc(mysqlQuery("SELECT * FROM sp_contragents WHERE kodcontragent='{$kodzakaz}'"));
            $outZak_name = !empty($_QRY_ContrAgent['namezaklong']) ? $_QRY_ContrAgent['namezaklong'] : $_QRY_ContrAgent['nameshort'];
            $outZak_address = $_QRY_ContrAgent['zakuraddress'];
            $outZak_dolg = $_QRY_ContrAgent['zakdolg'];
            $outZak_fio = $_QRY_ContrAgent['zakfio'];
            $outZak_inn = $_QRY_ContrAgent['zakinn'];
