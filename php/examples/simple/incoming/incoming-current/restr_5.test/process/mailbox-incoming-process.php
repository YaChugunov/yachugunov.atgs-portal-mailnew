...
function fixLog($db, $action, $id, $values) {
    ...
    if ("UPD" == $action) {
         ...
         */
                if ('inbox_docDate' == $key) {
                ...
                } elseif ('inbox_docDateDeadline' == $key) {
                ...
                } elseif ('inbox_docSourceDate' == $key) {
                    $oldVal = empty($oldSettings[$key]) ? "Нет даты" : (validateDate($oldSettings[$key], "Y-m-d") ? date('d.m.Y', strtotime($oldSettings[$key])) : "Нет даты");
                    $newVal = empty($value) ? "Нет даты" : (validateDate($value, "Y-m-d") ? date('d.m.Y', strtotime($value)) : "Нет даты");
                } elseif ('inbox_docSender_kodzakaz' == $key) {
                ...
                } elseif ('inbox_docRecipient_kodzayvtel' == $key) {
                ...
                } elseif ('inbox_docContractor_kodzayvispol' == $key) {
                    //
                ...
                } elseif ('inbox_controlIspolCheckout' == $key) {
                    //
                    ...
                    $newVal = $value;
                } elseif ('outbox_rowIDadd_rel' == $key) {
                    ...
                    foreach ($arrRowIDaddRel_old as $tmpVal1) {
                        ...
                        $resVal_old .= !empty($reqDB_old) ? $reqDB_old[0]['outbox_docID'] . "," : "";
                    }
                    ...
                    foreach ($arrRowIDaddRel_new as $tmpVal2) {
                        ...
                        $resVal_new .= !empty($reqDB_new) ? $reqDB_new[0]['outbox_docID'] . "," : "";
                    }
                    $newVal = rtrim($resVal_new, ",");
                } elseif ('inbox_controlIspolUseDeadline' == $key) {
...
function updateispolStatusOther($db, $id, $values) {
    ...
    //
    if ('1' == $values['ispolStatusOtherOff']) {
        if ("" != $koddocmail && "" != $kodispol) {
    ...
    //
    if ('1' == $values['ispolStatusOtherOn']) {
        if ("" != $koddocmail && "" != $kodispol) {
            $arrContractorID = explode(",", $__reqKoddocmail[0]['inbox_docContractorID']);
            foreach ($arrContractorID as $value) {
                //
...
function clearRelFields($db, $id, $values) {
    ...
    if ($_REQ_1 && "" != $_REQ_1[0]['outbox_rowID_rel'] && null != $_REQ_1[0]['outbox_rowID_rel']) {
        $outbox_rowID_rel  = $_REQ_1[0]['outbox_rowID_rel'];
        $outbox_rowID_relV = $values[__MAIL_INCOMING_TABLENAME]['outbox_rowID_rel'];
        $inbox_docTypeV    = $values[__MAIL_INCOMING_TABLENAME]['inbox_docType'];
        if (empty($outbox_rowID_relV) && !empty($outbox_rowID_rel)) {
            ...
            $docTYPEprev = !empty($_REQ_2) ? $_REQ_2[0]['outbox_docType_prev'] : null;
            ...
            if ($inbox_docTypeV < 2 || empty($outbox_rowID_relV)) {
                $db->update(__MAIL_OUTGOING_TABLENAME, array(
    ...
    if ('CRT' == $action) {
        ...
        if (!empty($outbox_rowID_rel) and $inbox_docType >= 2) {
            ...
            if (isset($_REQ_Outgoing) && !empty($_REQ_Outgoing)) {
                ...
                if (3 == $_REQ_Outgoing[0]['outbox_docType']) {
                    $db->update(__MAIL_OUTGOING_TABLENAME, array(
        ...
        if ('000000000000000' === $row[__MAIL_INCOMING_TABLENAME]['inbox_docSender_kodzakaz']) {
            $userID            = $_SESSION['id'];
            $userLastname      = $_SESSION['lastname'];
            $newKodcontragentm = newKodcontragentm();
            $senderName        = $row[__MAIL_INCOMING_TABLENAME]['inbox_docSender'];
            ...
            if ('save' == $saveManualInput) {
                $timecode = date('Y-m-d H:i:s');
                $req      = $db->sql("INSERT INTO sp_contragents_manualinput (timecode, kodcontragentm, nameshort, namefull, fromService, fromID, byUserID, byUserLastname, kfsim) VALUES ('$timecode', '$newKodcontragentm', '$senderName', '$senderName', 'mailnew inc', '$__newKod', '$userID', '$userLastname', '$percent')");
                if ($req) {
                    $db->update(
                        __MAIL_INCOMING_TABLENAME,
                        array(
                            'inbox_docSender_kodzakazm' => $newKodcontragentm,
                        ),
                        array(
                            'id' => $id,
                        )
            ...
            if (null !== $row[__MAIL_INCOMING_TABLENAME]['inbox_docSender_kodzakazm']) {
                $db->update(
                    __MAIL_INCOMING_TABLENAME,
                    array(
                        'inbox_docSender_kodzakazm' => null,
                        'inbox_docSender' => "",
                    ),
                    array(
                        'id' => $id,
        ...
        if ('000000000000000' === $row[__MAIL_INCOMING_TABLENAME]['inbox_docSender_kodzakaz']) {
            ...
            if (null !== $row[__MAIL_INCOMING_TABLENAME]['inbox_docSender_kodzakazm']) {
                $kodcontragent = $row[__MAIL_INCOMING_TABLENAME]['inbox_docSender_kodzakaz'];
                $inbox_docSender = "";
                if (!empty($kodcontragent)) {
                    $req_spContr = $db->sql("SELECT nameshort, namefull FROM sp_contragents WHERE kodcontragent='{$kodcontragent}'")->fetchAll();
                    $inbox_docSender = $req_spContr[0]['nameshort'];
                }
                $db->update(
                    __MAIL_INCOMING_TABLENAME,
                    array(
                        'inbox_docSender_kodzakazm' => null,
                        'inbox_docSender' => $inbox_docSender,
                    ),
                    array(
                        'id' => $id,
                    )
...
#
# UPD 28.09.23
# Список пользователей для оповещения о документе
#
##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### #####
#
#
function updateUserListNotify($db, $action, $id, $values) {
    $reqDB = $db->sql("SELECT docmailext, inbox_controlIspolMailUserListNotifyDL FROM " . __MAIL_INCOMING_TABLENAME . " WHERE id=" . $id)->fetchAll();
    #
    // Формируем список ID пользователей для уведомления
    $_DLUserIDs_DB  = "";
    $_DLUserIDs_DB  = $reqDB[0]['inbox_controlIspolMailUserListNotifyDL'];
    $_tmp_DB        = $reqDB[0]['docmailext'];
    $_DLUserIDs_New = $_DLUserIDs_DB;
    if ('1' == $values['checkbox_controlIspolMailUserListNotifyDL']) {
        $pos = strpos($_DLUserIDs_DB, $_SESSION['id']);
        if (false === $pos) {
            $_DLUserIDs_New = empty($_DLUserIDs_DB) ? $_SESSION['id'] : $_DLUserIDs_DB . "," . $_SESSION['id'];
        }
    } else {
        $pos = strpos($_DLUserIDs_DB, $_SESSION['id']);
        if (false !== $pos) {
            $_DLUserIDs_New = str_replace($_SESSION['id'], "", $_DLUserIDs_DB);
            $_DLUserIDs_New = str_replace(",,", ",", $_DLUserIDs_New);
            $_DLUserIDs_New = rtrim($_DLUserIDs_New, ",");
            $_DLUserIDs_New = ltrim($_DLUserIDs_New, ",");
        }
    }
    $db->update(__MAIL_INCOMING_TABLENAME, array(
        'inbox_controlIspolMailUserListNotifyDL' => $_DLUserIDs_New,
        'docmailext'                             => $values['checkbox_controlIspolMailUserListNotifyDL'] . " > " . $_SESSION['id'] . " > " . $_DLUserIDs_DB . " > NEW='" . $_DLUserIDs_New . "'",
    ), array(
        'id' => $id,
    ));
}
...
function updateSyslogAfterupload($db, $id) {
    // $_REQ1 = $db->sql("SELECT koddocmail FROM " . __MAIL_INCOMING_FILES_TABLENAME . " WHERE id=" . $id)->fetchAll();
    // $_koddocmail    = !empty($_REQ1[0]['koddocmail']) ? $_REQ1[0]['koddocmail'] : "";
    // $_REQ2 = $db->sql("SELECT inbox_docID FROM " . __MAIL_INCOMING_TABLENAME . " WHERE koddocmail=" . $_koddocmail)->fetchAll();
    // $_docID         = !empty($_REQ2[0]['inbox_docID']) ? $_REQ2[0]['inbox_docID'] : "";
...
function sendemailtoispolFullcheckout($db, $action, $id, $values, $row) {
    ...
    $__contrSUB        = $_sqlReq_1[0]['inbox_controlIspolMailUserListNotifyCheckout'];
    ...
    if ("1" == $__controlCheckout && ("1" == $_sqlReq_1[0]['inbox_controlIspolMailNotifyCheckout'] or "1" == $values[__MAIL_INCOMING_TABLENAME]['inbox_controlIspolMailNotifyCheckout'] or !empty($__contrSUB))) {

        $_ispolList = str_replace(",", ", ", $__contrName);
        if (!empty($__contrSUB) && ("1" == $_sqlReq_1[0]['inbox_controlIspolMailNotifyCheckout'] or "1" == $values[__MAIL_INCOMING_TABLENAME]['inbox_controlIspolMailNotifyCheckout'])) {
            $__contrMERGE = $__contrID . "," . $__contrSUB;
        } elseif (!empty($__contrSUB) && "1" !== $_sqlReq_1[0]['inbox_controlIspolMailNotifyCheckout'] && "1" !== $values[__MAIL_INCOMING_TABLENAME]['inbox_controlIspolMailNotifyCheckout']) {
            $__contrMERGE = $__contrSUB;
        } elseif (empty($__contrSUB) && ("1" == $_sqlReq_1[0]['inbox_controlIspolMailNotifyCheckout'] or "1" == $values[__MAIL_INCOMING_TABLENAME]['inbox_controlIspolMailNotifyCheckout'])) {
            $__contrMERGE = $__contrID;
        } else {
            $__contrMERGE = "";
        }
        $arrispol     = explode(",", $__contrMERGE);
        $arrFileIDAdd = explode(",", $__docFileIDadd);
        foreach ($arrispol as $value) {
            $__contractorDATA1 = $db->sql("SELECT id, emailaddress, namezayvfio FROM mailbox_sp_users WHERE id=" . $value)->fetchAll();
            $__contractorDATA2 = $db->sql("SELECT firstname, middlename, lastname FROM users WHERE id=" . $__contractorDATA1[0]['id'])->fetchAll();
            $__fileName        = "";
            $__fileURL         = "";
            $__fileLink        = "---";
            ...
            if ($__contractorDATA1 && $__contractorDATA2) {
                ...
                if (!empty($__contrSUB) && ("1" == $_sqlReq_1[0]['inbox_controlIspolMailNotifyCheckout'] or "1" == $values[__MAIL_INCOMING_TABLENAME]['inbox_controlIspolMailNotifyCheckout'])) {
                    ...
                    $message .= makeMailMsg_Notify_subscribeFullCheckout('incoming', $__koddocmail, $_msgTitle, $__docID, $__IO, $__fileLink, $__fileAddLinks, $__docTYPE, $__senderORG, $__docABOUT, $__sourceID, $_sourceDate, $__controlActive, $__controlStatus, $_docDeadline, $_ispolList);
                } else {
                    $message .= makeMailMsg_Notify_FullCheckout('incoming', $__koddocmail, $_msgTitle, $__docID, $__IO, $__fileLink, $__fileAddLinks, $__docTYPE, $__senderORG, $__docABOUT, $__sourceID, $_sourceDate, $__controlActive, $__controlStatus, $_docDeadline, $_ispolList);
                }
                #
...
function updateIspolCheckout($db, $id, $values, $row) {
    ...
    if (($cntIspol == $cntCheckouts) && ("1" != $row[__MAIL_INCOMING_TABLENAME]['inbox_controlIspolCheckout'])) {
...
function newUserMessage($db, $action, $id, $values) {
    ...
    if ("changed" == $_SESSION['inbox_checkChanges_is']) {
        if ((!empty($_QRY[0]['inbox_docContractorID']) && "1" != $_QRY[0]['inbox_docContractorID']) && (!empty($_QRY[0]['inbox_docContractor_kodzayvispol']) && "000000000000000" != $_QRY[0]['inbox_docContractor_kodzayvispol'])) {
            $msg_maintext    = '';
            $msg_subtext1    = '';
            $msg_subtext2    = '';
            $msg_subtext3    = '';
            ...
            if ("1" == $_QRY[0]['inbox_docContractorMULTI']) {
                $for_singleuser = "";
            ...
            $msg_title = ("CRT" == $action) ? "Новый входящий документ" : "Изменения по входящему документу";
...
function syncRelFields($db, $action, $id, $values, $row) {
    if ('UPD' == $action || 'CRT' == $action) {
        if ($values[__MAIL_INCOMING_TABLENAME]['outbox_rowIDadd_rel'] !== $row[__MAIL_INCOMING_TABLENAME]['outbox_rowIDadd_rel']) {
            $_koddocmail             = $row[__MAIL_INCOMING_TABLENAME]['koddocmail'];
            $_outboxRowIDaddRelArray = explode(',', $row[__MAIL_INCOMING_TABLENAME]['outbox_rowIDadd_rel']);
            if (!empty($_outboxRowIDaddRelArray)) {
                foreach ($_outboxRowIDaddRelArray as $value) {
                    $reqRel = $db->sql("SELECT inbox_rowIDadd_rel FROM " . __MAIL_OUTGOING_TABLENAME . " WHERE id='{$value}'")->fetchAll();
                    if ($reqRel) {
                        $replaceStr = str_replace($id, '', $reqRel[0]['inbox_rowIDadd_rel']);
                        $replaceStr = str_replace(',,', '', $replaceStr);
                        //
                        //
                        $db->update(__MAIL_OUTGOING_TABLENAME, array(
                            'inbox_rowIDadd_rel' => $replaceStr,
                        ), array(
                            'id' => $value,
                        ));
                    }
...
######
######
function saveContragentOnManualInput($db, $action, $id, $values, $row) {
    $koddocmail = $row[__MAIL_INCOMING_TABLENAME]['koddocmail'];
    if ('000000000000000' === $row[__MAIL_INCOMING_TABLENAME]['inbox_docSender_kodzakaz']) {
        if ("CRT" == $action) {
            $newKodcontragentm = newKodcontragentm();
            $senderName        = $row[__MAIL_INCOMING_TABLENAME]['inbox_docSender'];
            $req               = $db->sql("INSERT INTO sp_contragents_manualinput (kodcontragentm, nameshort, namefull, fromKoddocmail) VALUES ('$newKodcontragentm', '$senderName', '$senderName', '$koddocmail')");
            if ($req) {
                $db->update(
                    __MAIL_INCOMING_TABLENAME,
                    array(
...
Editor::inst($db, __MAIL_INCOMING_TABLENAME)
    ->fields(
        ...
        Field::inst(__MAIL_INCOMING_TABLENAME . '.inbox_docSender_kodzakaz')
            ->options(
                Options::inst()
                    ...
                    ->render(function ($row) {
                        $name = "";
                        if ("000000000000000" !== $row['kodcontragent']) {
                            if ("" != $row['namefull']) {
                            ...
                            $name = "В справочнике отсутствует (введено вручную)";
                        }
                        return $name;
                    })
                    ->where(function ($q) {
        ...
        // ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
        Field::inst(__MAIL_INCOMING_TABLENAME . '.inbox_docSender_kodzakazm')
            ->options(
                Options::inst()
                    ->table('sp_contragents_manualinput')
                    ->value('kodcontragentm')
                    ->label(array('kodcontragentm', 'namefull', 'nameshort'))
                    ->render(function ($row) {
                        $name = "";
                        if ("" != $row['namefull']) {
                            $name = $row['namefull'] . " (полное название)";
    ...
    ->on('preEdit', function ($editor, $id, $values) {
        ...
        updateLogCheckouts($editor->db(), $id, $values);
        // updateUserListNotify($editor->db(), 'UPD', $id, $values);
    })
    ...
    ->on('postCreate', function ($editor, $id, $values, $row) {
        ...
        saveContragentOnManualInput($editor->db(), 'CRT', $id, $values, $row);
    })
    ->on('postEdit', function ($editor, $id, $values, $row) {
        ...
        // updateUserListNotify($editor->db(), 'UPD', $id, $values);
        //saveContragentOnManualInput($editor->db(), 'UPD', $id, $values, $row);
    })
    ...
    ->where(__MAIL_INCOMING_TABLENAME . '.koddel', '99', '!=')
