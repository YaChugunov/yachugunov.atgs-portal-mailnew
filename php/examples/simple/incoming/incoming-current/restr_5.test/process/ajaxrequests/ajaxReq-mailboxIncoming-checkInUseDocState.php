...
if (isset($_SESSION['password']) && isset($_SESSION['login'])) {
	if (checkUserAuthorization($_SESSION['login'], $_SESSION['password']) == -1) {
		...
		if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && "" != $_koddocmail) {
			if ($_action == "check") {
				$_reqDB = mysqli_fetch_array(mysqlQuery("SELECT inbox_docID, inuse_flag, inuse_userid FROM " . __MAIL_INCOMING_TABLENAME . " WHERE koddocmail = '{$_koddocmail}'"));
				if (!empty($_reqDB)) {
					$docID = $_reqDB['inbox_docID'];
				...
				if (!empty($_reqDB)) {
					...
					$inUseUserLastname = "";
					$inUseUserFirstname = "";
					$_reqDB1 = mysqlQuery("UPDATE " . __MAIL_INCOMING_TABLENAME . " SET inuse_flag='1', inuse_userid='{$_userid}' WHERE koddocmail = '{$_koddocmail}'");
					if ($_reqDB1) {
						$output_response = json_encode([$docID, $inUseFlag, $inUseUserID, $inUseUserLastname, $inUseUserFirstname]);
					} else {
				...
				if (!empty($_reqDB)) {
					...
					$inUseUserLastname = "";
					$inUseUserFirstname = "";
					$_reqDB1 = mysqlQuery("UPDATE " . __MAIL_INCOMING_TABLENAME . " SET inuse_flag='0', inuse_userid='' WHERE koddocmail = '{$_koddocmail}'");
					if ($_reqDB1) {
						$output_response = json_encode([$docID, $inUseFlag, $inUseUserID, $inUseUserLastname, $inUseUserFirstname]);
					} else {
				...
				$_reqDB1 = mysqlQuery("UPDATE " . __MAIL_INCOMING_TABLENAME . " SET inuse_flag='0', inuse_userid='' WHERE inuse_userid = '{$_userid}'");
				if ($_reqDB1) {
					$output_response = "unset all ok";
				} else {
