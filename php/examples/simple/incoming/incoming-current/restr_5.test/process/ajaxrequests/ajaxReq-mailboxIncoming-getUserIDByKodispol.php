...
$_kodispol = isset($_POST['kodispol']) ? $_POST['kodispol'] : "";
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
...
if (isset($_SESSION['password']) && isset($_SESSION['login'])) {
	if (checkUserAuthorization($_SESSION['login'], $_SESSION['password']) == -1) {
		...
		if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && $_kodispol != "") {
			$_reqDB = mysqli_fetch_array(mysqlQuery("SELECT ID FROM mailbox_sp_users WHERE kodispol = '{$_kodispol}'"));
			$output = $_reqDB['ID'];
		} else {
			$output = -1;
		}
