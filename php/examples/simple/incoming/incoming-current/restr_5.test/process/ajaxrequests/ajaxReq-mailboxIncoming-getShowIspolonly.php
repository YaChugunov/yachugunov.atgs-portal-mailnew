	...
	if (checkUserAuthorization($_SESSION['login'], $_SESSION['password']) == -1) {
		...
		if (checkUserRestrictions_defaultDB($_SESSION['id'], 'mail', 3, 0) == 1) {
			if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
				$_QRY1 = mysqli_fetch_array(mysqlQuery("SELECT showispolonly FROM mailbox_sp_users WHERE id='" . $userID . "'"));
				$_QRY2 = mysqli_fetch_array(mysqlQuery("SELECT incoming_showIspolOnly FROM mailbox_userSettingsUI WHERE id='" . $userID . "'"));
				if ($_QRY1 && $_QRY2) {
					// $output = $_QRY1['showispoloutonly'];
					$output = $_QRY2['incoming_showIspolOnly'];
				} else {
					$output = '0';
				}
			} else {
