...
// ID записи в таблице ".__MAIL_INCOMING_PREFIX."_files в виде "row_XXXXX"
$_rowID = isset($_POST['rowID']) && $_POST['rowID'] != "" ? str_replace('row_', '', $_POST['rowID']) : "";
// Номер документа в таблице ".__MAIL_INCOMING_PREFIX."
		...
		if (!empty($file['error']) || empty($file['tmp_name'])) {
			...
			if (empty($name) || empty($parts['extension'])) {
				...
				if (move_uploaded_file($file['tmp_name'], $path . $__name)) {
					...
					// >>> 
					// Редакция от 09.01.24
					$file_url = str_replace($_SERVER['DOCUMENT_ROOT'], 'http://' . $_SERVER['HTTP_HOST'], $varFileArray['syspath'] . $__pref . "-" . $file_newOriginalName);
					...
					// 
					$insertRowToFilesTable = $db_handle->runQuery("INSERT INTO " . __MAIL_INCOMING_FILES_TABLENAME . " (koddocmail, mainfile, flag, file_year, file_id, file_name, file_originalname, file_extension, file_symname, file_size, file_truelocation, file_syspath, file_webpath, file_url) VALUES ('{$_koddocmail}', '0', 'PREUPL', '{$file_year}', '{$file_id}', '{$file_name}', '{$file_originalname}', '{$file_extension}', '{$file_symname}', '{$file_size}', '{$file_truelocation}', '{$file_syspath}', '{$file_webpath}', '{$file_url}')");
					// $newFileID = mysqli_insert_id($_QRY_SaveToDB);
					$addedRowID = $db_handle->lastInsertID();
					$inbox_docFileIDadd .= $addedRowID . ",";
					// 
