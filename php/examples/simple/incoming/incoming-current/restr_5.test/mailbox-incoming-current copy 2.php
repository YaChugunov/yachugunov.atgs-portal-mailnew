...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready(function() {
        ...
        editor_incoming.on('initEdit', function(e, node, data, items, type) {
            ...
            //
            $.ajax({
                url: "<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR5; ?>/process/mailbox-incoming-process-loadOutgoing.php",
                type: 'post',
                dataType: 'json',
                success: function(json) {
                    console.log('ajax mailbox-incoming-process-loadOutgoing', json);
                    editor_incoming.add({
                        label: "Основное исходящее письмо, для которого редактируемое будет ответным ( Дата / № АТГС / № орг / Название орг / Тема )",
                        name: "<?php echo __MAIL_INCOMING_TABLENAME; ?>.outbox_rowID_rel",
                        type: "select",
                ...
                language: {
                    ...
                    noResults: function() {
                        setTimeout(function() {
                            ...
                            $('#customForm-mail-main-inbox .sender-manualinput').fadeIn(
                                'slow').css('display', 'block');
                        }, 500);
                        return 'Ничего не найдено';
                    },
                    searching: function() {
                        $('#customForm-mail-main-inbox .sender-manualinput').fadeOut('slow')
                            .css('display', 'none');
                        editor_incoming.field(
                                '<?php echo __MAIL_INCOMING_TABLENAME; ?>.inbox_docSender')
                            .hide(false);
