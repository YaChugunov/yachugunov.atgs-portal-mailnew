...
$(document).ready(function() {
    ...
    editor_incoming.on('preOpen', function(e, mode, action) {
        ...
        if (action === 'edit') {
            ...
            if (editor_incoming.field(
                ...
                "000000000000000") {
                console.log("CHECK POINT #2147 inbox_docSender_kodzakaz >>>", editor_incoming.field(
                    '<?php echo __MAIL_INCOMING_TABLENAME; ?>.inbox_docSender_kodzakaz').val());
                setTimeout(function() {
                    $('#customForm-mail-main-inbox .sender-manualinput-chk').fadeIn(
                            'slow')
                        .css('display', 'block');
                    $('#customForm-mail-main-inbox .sender-manualinput-text').fadeIn(
                            'slow')
                        .css('display', 'block');
                    // Отображаем поле ввода организации
                    editor_incoming.field(
                            '<?php echo __MAIL_INCOMING_TABLENAME; ?>.inbox_docSender')
                        .show(false);
                    // Отображаем чекбокс ручного ввода
                    editor_incoming.field('enblSenderManual').show(false);

                    $('#customForm-mail-main-inbox .sender-manualinput-alert').css('display',
                        'block');
                }, 500);
            } else {
                $('#customForm-mail-main-inbox .sender-manualinput-alert').css('display',
            ...
            language: {
                ...
                searching: function() {
                    if (editor_incoming.field(
                        ...
                        $('#customForm-mail-main-inbox .sender-manualinput-alert').css(
                            'display', 'none');
                        $('#customForm-mail-main-inbox .sender-manualinput-chk').fadeOut(
                                'slow')
                            .css('display', 'none');
    ...
    editor_incoming.on('preSubmit', function(e, data, action) {
        ...
        if (action !== "remove") {
            ...
            if (editor_incoming.field(
                ...
                .text() === 'В справочнике отсутствует (ручной ввод)' && $(
    ...
    editor_incoming.on('postCreate postEdit close', function(e) {
        ...
        if (editor_incoming.field(
                '<?php echo __MAIL_INCOMING_TABLENAME; ?>.inbox_docSender_kodzakaz').val() ===
            "000000000000000") {
            console.log('inbox_docSender_kodzakaz', editor_incoming.field(
                '<?php echo __MAIL_INCOMING_TABLENAME; ?>.inbox_docSender_kodzakaz').val());
    ...
    editor_incoming.dependent('enblSenderManual', function(val, data, callback, e) {
        ...
        if (val == "1") {
            ...
            $('#manualInput-searchResults').html('');
            $('#manualInput-searchResults').css({
                "display": "none"
            });
        }
        callback(true);
            ...
            },
            {
                ...
                render: function(data, type, row, meta) {
                    ...
                    if (kodzakaz === "000000000000000") {
                        manualInputIcon =
                            '<span data-toggle="tooltip" data-placement="top" title="Название контрагента ручной ввод. Возможно просто кто-то не счел нужным его поискать в Справочнике и сделать доступным в Почте, или же добавить как новую оргнизацию." class="float-left"><sup><i class="fa-solid fa-star-of-life fa-xs text-danger"></i><i class="fa-solid fa-star-of-life fa-xs text-danger mr-1"></i></sup></span>';
                        manualInputClass = '';
                    }
    ...
    $(document).on("keyup", "input#DTE_Field_mailbox_incoming-inbox_docSender", delay(function(e) {
        ...
        if (checkOnSimilar.length > 0) {
            $('#manualInput-searchResults').css({
                "display": "block"
            });
            if (checkOnSimilar.length > 20) {
                $('#manualInput-searchResults').append(
                    '<div class="alert alert-danger alert-fadein fade show w-100 text-center d-block" role="alert">Найдено более 20 записей. Введите еще слово для поиска.</div>'
                );
            } else {
                $('#manualInput-searchResults').append(
                    '<div class="title mx-0 mb-2">Результаты контекстного поиска</div>');
                checkOnSimilar.forEach(function(element, key) {
                    let logItem = key + ': "' + element.text + '" >>> ' + element.nameshort;
                    let itemShortName = element.nameshort;
                    let itemFullName = element.namefull;
                    let itemKod = element.kodcontragent;
                    let itemKodm = element.kodcontragentm;
                    if (checkVal(itemKod) && !checkVal(itemKodm)) {
                        $('#manualInput-searchResults').append(
                            '<span class="itemPopover mx-1" data-toggle="popover" data-content="<div>' +
                            itemFullName +
                            '</div>"><a href="#" class="badge badge-light py-1" data-namecontragent="' +
                            itemShortName + '" data-kodcontragent="' +
                            itemKod + '" data-kodcontragentm="' + itemKodm + '">' +
                            itemShortName + '</a></span>');
                    } else if (!checkVal(itemKod) && checkVal(itemKodm)) {
                        $('#manualInput-searchResults').append(
                            '<span class="itemPopover mx-1" data-toggle="popover" data-content="<div>' +
                            itemFullName +
                            '</div>"><a href="#" class="badge badge-warning py-1" data-namecontragent="' +
                            itemShortName + '" data-kodcontragent="' +
                            itemKod + '" data-kodcontragentm="' + itemKodm + '">' +
                            itemShortName + '</a></span>');
                    }
                    $('#manualInput-searchResults > span.itemPopover').popover({
                        html: true,
                        trigger: 'hover',
                        placement: 'top',
                        container: '#sender-manualinput-alert',
                        customClass: 'manualInput-searchResult-popover'
                    });
                });
            }
        } else {
    ...
    // ###### ## ##### ## ###### ## ##### ## ###### ## ##### ## ###### ## #####
    // 
    $(document).on("click", "#manualInput-searchResults > span.itemPopover", function() {
        el = $(this);
        ela = $(this).children('a');
        elaKod = ela.attr('data-kodcontragent');
        elaKodm = ela.attr('data-kodcontragentm');
        elaName = ela.attr('data-namecontragent');
        checkElaKod = checkVal(elaKod) && (elaKod != 'null') ? 1 : 0;
        checkElaKodm = checkVal(elaKodm) && (elaKodm != 'null') ? 1 : 0;
        if (checkElaKod && !checkElaKodm) {
            console.log('#manualInput-searchResults clicked! v1', 'data-kodcontragent >', elaKod,
                'data-kodcontragentm >', elaKodm, 'data-namecontragent >', elaName, checkElaKod,
                checkElaKodm);
            editor_incoming.field('<?php echo __MAIL_INCOMING_TABLENAME; ?>.inbox_docSender_kodzakaz')
                .val(elaKod);
            editor_incoming.field('<?php echo __MAIL_INCOMING_TABLENAME; ?>.inbox_tmp2').val(elaName);
            editor_incoming.field('<?php echo __MAIL_INCOMING_TABLENAME; ?>.inbox_docSender').val(
                elaName);
            editor_incoming.field('<?php echo __MAIL_INCOMING_TABLENAME; ?>.inbox_docSender').disable();
            editor_incoming.field('enblSenderManual').val(0);
            $('input#DTE_Field_enblSenderManual_0').prop('checked', false);
        } else if (!checkElaKod && checkElaKodm) {
            console.log('#manualInput-searchResults clicked! v2', 'data-kodcontragent >', elaKod,
                'data-kodcontragentm >', elaKodm, 'data-namecontragent >', elaName, checkElaKod,
                checkElaKodm);
            editor_incoming.field('<?php echo __MAIL_INCOMING_TABLENAME; ?>.inbox_docSender_kodzakazm')
                .val(elaKodm);
...
<link rel="stylesheet"
      href="<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR5; ?>/css/common-modals.css">
<link rel="stylesheet"
      href="<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR5; ?>/css/common-customform.css">
<?php
// ----- ----- ----- ----- -----
...
?>
<link rel="stylesheet"
      href="<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR5; ?>/css/mailbox-incoming.css">
<link rel="stylesheet"
      href="<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR5; ?>/css/mailbox-incoming-details.css">
...
<section>
    ...
    <div id="divOnlyIspolMe" class="form-check checkbox mb-3">
        ...
        <label class="form-check-label ml-2" for="chkOnlyIspolMe" data-toggle="popover"
               data-content='<p>Данный чекбокс фиксирует вас как ответственного в блоке фильтров и может быть снят только повторным кликом. Установка сохраняется как персональная настройка сервиса в БД. Очистка фильтров данный чекбокс не изменит.</p>'>
            ...
            <thead>
                <tr>
                    ...
                    <th data-toggle="tooltip"
                        title="<p>Если вы видите какой-либо символ в этом столбце - контроль исполнения (КИ) включен. Расшифровку символа можно поссмотреть в легенде под таблице с документами, либо наведя указатель мыши на сам символ.</p>">
                        КИ</th>
                    <th data-toggle="tooltip" title="<p>Основной прикрепленный файл к документу.</p>"><span
                              class="">Ф</span><span class="small" style="font-size:65%">o</span></th>
                    <th data-toggle="tooltip"
                        title="<p>Дполнительные прикрепленные файл(ы) к документу. Посмотреть список этих файлов можно во всплывающем окне кликнув на иконке в этом столбце. Допускается любое количество файлов и любых типов.</p>">
                        <span class="">Ф</span><span class="small" style="font-size:65%">д</span>
                    <th data-toggle="tooltip"
                        title="<p>Ссылка в виде номера документа в исходящих, который является ответным либо запросом ответа по отношению к документу в строке списка.</p>">
                        <span class="">ЗО</span><span class="small" style="font-size:65%">o</span>
                    </th>
                    <th data-toggle="tooltip"
                        title="<p>Дополнительные исходящие документы, которые были отмечены как те, на которые данный документ является ответным. Посмотреть список этих документов можно во всплывающем окне кликнув на иконке в этом столбце.</p>">
                        <span class="">ЗО</span><span class="small" style="font-size:65%">д</span>
                    </th>
                    <th data-toggle="tooltip"
                        title="<p>Списки связанных входящих и исходящих документов, а также договоров и контрагентов, которые были отмечены как связанные с данным документом. Эти связи устанавливаются вручную при редактировании документа (вкладка Связи).</p>">
                        СВ</th>
                    <th data-toggle="tooltip" title="<p>Журнал действий с документом.</p>">Ж</th>
                    <th data-toggle="tooltip"
                        title="<p>Чат по документу. Можно оставлять сообщения как непосредственно в чате, так и через форму редактирвоания. Комментарий исполнителя и дополнительная инофрмация о документе из формы редактирования также появится в этом чате.</p>">
                        К</th>
                    <th>17</th>
                    <th>18</th>
                    <th>19</th>
...
<div class="modal fade" data-backdrop="true" id="modal-noControlIspolActive" tabindex="-1" role="dialog"
     aria-labelledby="modal-noControlIspolActive-label" aria-hidden="true">
...
; ?>
<link rel="stylesheet"
      href="<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR5; ?>/css/mailbox-incoming-modal-logCheckouts.css">
<div class="modal fade" data-backdrop="true" id="modal-logCheckouts" tabindex="-1" role="dialog"
     aria-labelledby="modal-logCheckouts-label" aria-hidden="true">
    <div class="modal-dialog modal-lg custom-modal" role="document">
        <div class="modal-content shadow">
            ...
            <div class="modal-body">
                <div class="container-fluid">
                    <table id="table-logCheckouts" class="table table-striped table-borderless" cellspacing="0"
                           width="100%">
...
; ?>
<link rel="stylesheet"
      href="<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR5; ?>/css/mailbox-incoming-modal-logChanges.css">
<div class="modal fade" data-backdrop="true" id="modal-logChanges" tabindex="-1" role="dialog"
     aria-labelledby="modal-logChanges-label" aria-hidden="true">
    <div class="modal-dialog modal-lg custom-modal" role="document">
        <div class="modal-content shadow">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <div class="container-fluid">
                    <table id="table-logChanges" style="margin-bottom:20px !important"
                           class="table table-striped table-borderless" cellspacing="0" width="100%">
...
; ?>
<link rel="stylesheet"
      href="<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR5; ?>/css/mailbox-incoming-modal-logComments.css">
<div class="modal fade" data-backdrop="true" id="modal-logComments" tabindex="-1" role="dialog"
     aria-labelledby="modal-logComments-label" aria-hidden="true">
    <div class="modal-dialog modal-lg custom-modal" role="document">
        <div class="modal-content shadow">
            <div class="modal-header">
...
; ?>
<link rel="stylesheet"
      href="<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR5; ?>/css/mailbox-incoming-modal-listAddFiles.css">
<div class="modal fade" data-backdrop="true" id="modal-listAddFiles" tabindex="-1" role="dialog"
     aria-labelledby="modal-listAddFiles-label" aria-hidden="true">
    <div class="modal-dialog modal-lg custom-modal" role="document">
        <div class="modal-content shadow">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <div class="container-fluid">
                    ...
                    <table id="table-listAddFiles" class="table table-borderless table-striped mb-3" cellspacing="0"
                           width="100%">
                        <thead class="thead-dark" style="display:none">
                            <tr>
                                <th>Имя файла</th>
...
; ?>
<link rel="stylesheet"
      href="<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR5; ?>/css/mailbox-incoming-modal-listLinkDocs.css">
<div class="modal fade" data-backdrop="true" id="modal-listLinkDocs" tabindex="-1" role="dialog"
     aria-labelledby="modal-listLinkDocs-label" aria-hidden="true">
    <div class="modal-dialog modal-lg custom-modal" role="document">
        <div class="modal-content shadow">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <div class="container-fluid">
                    <div class="space20"></div>
                    <table id="table-listLinkDocs" class="table table-borderless table-striped" cellspacing="0"
                           width="100%">
                        <thead class="thead-dark">
                            <tr>
                                <th>№</th>
            ...
            <div id="wrap-linkedOutgoing-showFiles" style="display: none">
                <p class="mb-1"><strong>Прикрепленные файлы к документу № 1-1/<span
                              id="linkedOutgoing-docID"></span></strong></p>
...
; ?>
<link rel="stylesheet"
      href="<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR5; ?>/css/mailbox-incoming-modal-listAdditionalLinkDocs.css">
<div class="modal fade" data-backdrop="true" id="modal-listAdditionalLinkDocs" tabindex="-1" role="dialog"
     aria-labelledby="modal-listAdditionalLinkDocs-label" aria-hidden="true">
        ...
        <div class="modal-content shadow">
            ...
            <div class="modal-body">
                <div class="container-fluid">
                    ...
                    <table id="table-listAdditionalLinkDocsMInc" class="table table-borderless table-striped mb-3"
                    ...
                    <table id="table-listAdditionalLinkDocsMOut" class="table table-borderless table-striped mb-3"
                           cellspacing="0" width="100%">
                        <thead class="thead-dark" style="display:none">
                            <tr>
                                <th>№ исх</th>
                    ...
                    <table id="table-listAdditionalLinkDocsDog" class="table table-borderless table-striped"
                           cellspacing="0" width="100%">
                        <thead class="thead-dark" style="display:none">
                            <tr>
                                <th>№ дог</th>
                    ...
                    <table id="table-listAdditionalLinkDocsSP" class="table table-borderless table-striped"
                           cellspacing="0" width="100%">
                        <thead class="thead-dark" style="display:none">
                            <tr>
                                <th>ID</th>
