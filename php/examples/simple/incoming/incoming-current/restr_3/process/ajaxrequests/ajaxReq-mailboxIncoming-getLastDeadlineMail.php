...
if (isset($_SESSION['password']) && isset($_SESSION['login'])) {
	if (checkUserAuthorization($_SESSION['login'], $_SESSION['password']) == -1) {
		...
		if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && $_koddocmail != "") {
			$_QRY_getLastDeadlineMail = mysqli_fetch_array(mysqlQuery("SELECT * FROM " . __MAIL_INCOMING_PREFIX . "_logControl WHERE `action` = 'mail' AND `userid` = '{$_userid}' AND `koddocmail` = '{$_koddocmail}' ORDER BY `timestamp` DESC LIMIT 1"));
			$trigger = $_QRY_getLastDeadlineMail['controlTrigger'];
			$timestamp = $_QRY_getLastDeadlineMail['timestamp'];
			$output = $trigger . "/" . date('d.m.Y H:i:s', strtotime($timestamp));
		} else {
			$output = -1;
		}
	}
