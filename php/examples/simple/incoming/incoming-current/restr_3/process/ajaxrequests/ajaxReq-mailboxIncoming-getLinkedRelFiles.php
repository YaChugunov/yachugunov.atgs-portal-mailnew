...
if (isset($_SESSION['password']) && isset($_SESSION['login'])) {
	if (checkUserAuthorization($_SESSION['login'], $_SESSION['password']) == -1) {
		...
		if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && $_koddocmail != "") {
			$_reqDB_1 = mysqli_fetch_array(mysqlQuery("SELECT outbox_docFileID FROM " . __MAIL_OUTGOING_TABLENAME . " WHERE koddocmail = '{$_koddocmail}'"));
			$outbox_docFileID = $_reqDB_1['outbox_docFileID'];
			if ($outbox_docFileID != "" && $outbox_docFileID != NULL) {
				$_reqDB_2 = mysqli_fetch_array(mysqlQuery("SELECT * FROM " . __MAIL_OUTGOING_FILES_TABLENAME . " WHERE id = '{$outbox_docFileID}'"));
