...
function clearRelFields($db, $id, $values) {
    ...
    if ($_REQ_1 && $_REQ_1[0]['outbox_rowID_rel'] != "" && $_REQ_1[0]['outbox_rowID_rel'] != NULL) {
        ...
        if (empty($outbox_rowID_relV) && !empty($outbox_rowID_rel)) {
            ...
            if ($docTYPEprev != "" && $docTYPEprev != null) {
...
function updateFields($db, $action, $id, $values, $row) {
    ...
    if ('CRT' == $action) {
        ...
        if ($row[__MAIL_INCOMING_TABLENAME]['inbox_docSender_kodzakaz'] === '000000000000000') {
            $userID = $_SESSION['id'];
            ...
            if ($row[__MAIL_INCOMING_TABLENAME]['inbox_docSender_kodzakazm'] !== null) {
                $db->update(
                    __MAIL_INCOMING_TABLENAME,
                    array(
...
function updateUserListNotify($db, $action, $id, $values) {
    ...
    if ($values['checkbox_controlIspolMailUserListNotifyDL'] == '1') {
...
function sendEmailToIspol_FullCheckout($db, $action, $id, $values, $row) {
    ...
    if ($__controlCheckout == "1" && ($_sqlReq_1[0]['inbox_controlIspolMailNotifyCheckout'] == "1" or $values[__MAIL_INCOMING_TABLENAME]['inbox_controlIspolMailNotifyCheckout'] == "1" or !empty($__contrSUB))) {

        $_ispolList = str_replace(",", ", ", $__contrName);
        if (!empty($__contrSUB) && ($_sqlReq_1[0]['inbox_controlIspolMailNotifyCheckout'] == "1" or $values[__MAIL_INCOMING_TABLENAME]['inbox_controlIspolMailNotifyCheckout'] == "1")) {
            $__contrMERGE = $__contrID . "," . $__contrSUB;
        } elseif (!empty($__contrSUB) && $_sqlReq_1[0]['inbox_controlIspolMailNotifyCheckout'] !== "1" && $values[__MAIL_INCOMING_TABLENAME]['inbox_controlIspolMailNotifyCheckout'] !== "1") {
            $__contrMERGE = $__contrSUB;
        } elseif (empty($__contrSUB) && ($_sqlReq_1[0]['inbox_controlIspolMailNotifyCheckout'] == "1" or $values[__MAIL_INCOMING_TABLENAME]['inbox_controlIspolMailNotifyCheckout'] == "1")) {
