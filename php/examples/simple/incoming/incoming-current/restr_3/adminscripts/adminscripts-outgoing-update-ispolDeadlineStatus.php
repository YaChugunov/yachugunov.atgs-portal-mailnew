...
if (isset($_SESSION['password']) && isset($_SESSION['login'])) {
	if (checkUserAuthorization($_SESSION['login'], $_SESSION['password']) == -1) {
		...
		if ($_SESSION['id'] == '999') {
			...
			$reqMailSettings = mysqli_fetch_assoc(mysqlQuery("SELECT * FROM mailbox_systemSettings WHERE typeMailbox = 'outgoing'"));
			...
			$reqMainData = mysqlQuery("SELECT koddocmail, outbox_docDateDeadline, outbox_controlIspolActive, outbox_controlIspolUseDeadline FROM mailbox_outgoing WHERE outbox_docDate BETWEEN '2021-01-01 00:00:00' AND '2023-12-31 23:59:59'");
			while ($rowMainData = mysqli_fetch_assoc($reqMainData)) {
				$koddocmail = $rowMainData['koddocmail'];
				// Дедлайн по документу
				$dateDL = date("Y-m-d 00:00:00", strtotime($rowMainData['outbox_docDateDeadline']));
				if ($rowMainData['outbox_controlIspolActive'] == '1' && $rowMainData['outbox_controlIspolUseDeadline'] == '1') {
				...
				$reqUpdateStatusDL = mysqlQuery("UPDATE mailbox_outgoing SET outbox_controlIspolStatusDeadline='{$statusDL}' WHERE koddocmail='{$koddocmail}'");
