...
<script type="text/javascript" language="javascript" class="init">
    ...
    function ajaxRequest_saveToDB_showIspolOnly(responseHandler) {
        ...
        request_showIspolOnly = $.ajax({
            url: '<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR3; ?>/process/ajaxrequests/ajaxReq-mailboxIncoming-saveShowIspolonly.php',
            type: "post",
            cache: false,
            data: {
                userID: _userid,
    ...
    function ajaxRequest_loadFromDB_showIspolOnly(responseHandler) {
        ...
        // Fire off the request_addItem to /form.php
        request_showIspolOnly = $.ajax({
            url: '<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR3; ?>/process/ajaxrequests/ajaxReq-mailboxIncoming-getShowIspolonly.php',
            type: "post",
            cache: false,
            data: {
                userID: _userid
    ...
    function ajaxRequest_getRelativeOutgoingDataAsync(rowid) {
        ...
        $.ajax({
            ...
            cache: false,
            type: "post",
            url: '<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR3; ?>/process/ajaxrequests/ajaxReq-mailboxIncoming-getRelativeOutgoingData.php',
            data: {
                rowid: rowid
            },
    ...
    function ajaxRequest_checkInUseDocStateDataAsync(action, koddocmail) {
        ...
        $.ajax({
            ...
            cache: false,
            type: "post",
            url: '<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR3; ?>/process/ajaxrequests/ajaxReq-mailboxIncoming-checkInUseDocState.php',
            data: {
                action: action,
                koddocmail: koddocmail
            },
    ...
    $(document).ready(function() {
        editor_incoming = new $.fn.dataTable.Editor({
            ...
            fields: [{
                ...
                {
                    label: "Уведомить меня на email об исполнении по этому документу <span class='text-danger'>Скоро</span>",
                    type: "checkbox",
                ...
                {
                    label: "Уведомить меня на email о наступлении дедлайна по этому документу <span class='text-danger'>Скоро</span>",
                    type: "checkbox",
        ...
        editor_incoming.on('preOpen', function(e, mode, action) {
            ...
            if (action === 'edit') {
                ...
                if (userListNotifyDL.indexOf(userID) !== -1) {
                    ...
                    console.log('field checkbox_controlIspolMailUserListNotifyDL', editor_incoming
                        .field('checkbox_controlIspolMailUserListNotifyDL').val());
                } else {
                    $('input[id="DTE_Field_checkbox_controlIspolMailUserListNotifyDL_0"]').prop(
                        'checked', false);
                    editor_incoming.field('checkbox_controlIspolMailUserListNotifyDL').val(0);
                    console.log('checkbox_controlIspolMailUserListNotifyDL', userListNotifyDL.indexOf(
                        userID));
                    console.log('field checkbox_controlIspolMailUserListNotifyDL', editor_incoming
                        .field('checkbox_controlIspolMailUserListNotifyDL').val());
                }
        ...
        editor_incoming.on('open', function(e, mode, action) {
            ...
            console.log("CHECK >>> .on(open) inbox_controlIspolActive", editor_incoming
                .field(
            ...
            console.log("CHECK >>> .on(open) ispolStatus", $(
                    'input[id="DTE_Field_ispolStatus_0"]')
                .val());
            ...
            function(val, data, callback, e) {
                if (val !== "" && val !== null) {
                    ...
                    if (relVal[6] !== 'no' && relVal[7] !== 'no' && relVal[8] !== 'no') {
                        ...
                        $('#select2-DTE_Field_mailbox_incoming-outbox_rowID_rel-container').addClass(
                            'text-danger');
                        $('input[id="DTE_Field_enbl_outbox_rowIDadd_rel_0"]').prop('checked', true);
                        editor_incoming.field('enbl_outbox_rowIDadd_rel').val(1);
                        editor_incoming.field(
                            '<?php echo __MAIL_INCOMING_TABLENAME; ?>.outbox_rowIDList_rel').val(relVal[0]);
                    } else {
                        $('div.outbox-relExist-alert').css('display', 'none');
                        $('div.outbox-relExist-alert .outbox-relExist-alert-msg').html('');
                        $('#select2-DTE_Field_mailbox_incoming-outbox_rowID_rel-container').removeClass(
                            'text-danger');
                        // $('input[id="DTE_Field_enbl_outbox_rowIDadd_rel_0"]').prop('checked', false);
                        // editor_incoming.field('enbl_outbox_rowIDadd_rel').val(0);
        ...
        editor_incoming.dependent(
            ...
            function(val, data, callback, e) {
                ...
                if (val == "1") {
                    ...
                    console.log("CHECKPOINT 2471 >>>", ispols, userid, checkArrOnString(ispols,
                        userid));
                    ...
                    if (userid == "999" || userid == "1011" || userid == "1114" || userid == "1067") {
                        editor_incoming.field(
                                '<?php echo __MAIL_INCOMING_TABLENAME; ?>.inbox_controlIspolMailSpecialNotifyDL'
                            )
                            .enable();
        ...
        editor_incoming.dependent(['ispolStatus', 'ispolStatusOtherOn', 'ispolStatusOtherOff'],
            function(val, data, callback, e) {
                console.log("CHECK >>> dependent ispolStatus", $('input[id="DTE_Field_ispolStatus_0"]')
                    .is(
                        ':checked'), val);
                if ($(
        ...
        table_incoming = $('#inbox').DataTable({
            ...
            createdRow: function(row, data, index) {
                ...
                //
                console.log('checkVal(relRowID)', checkVal(relRowID), 'checkVal(relRowIDadd)',
                    checkVal(
            ...
            buttons: [{
                    ...
                    action: function(e, dt, node, config) {
                        $('#mail-incoming-filters-block *').filter('input, select').each(
                            function(
                                index, element) {
                        ...
                        $('#mail-incoming-filters-block *').filter('input, select').each(
                            function(
                                index, element) {
                                sessionStorage.removeItem('inbox_' + $(element).attr(
                                    'id'));
                            });
        ...
        table_incoming.one('preDraw', function() {
            ...
            if (pageCurrent !== 'nodraw' && pageCurrent !== '' && pageCurrent !== null) {
                ...
                table_incoming.one('draw', function() {
                    ...
                    console.log('incoming_pageCurrent #3', 'Table is drawn', table_incoming
                        .page
                        .info().page);
                });
            }
                ...
                if ($("#chkOnlyIspolMe").prop('checked') === true) {
                    ...
                    $('#mail-incoming-filters-block *').filter('input, select').each(function(index,
                        ...
                        sessionStorage.setItem('inbox_' + $(element).attr('id'), $(element)
                            .val());
                        console.log(index, 'sessionStorage.getItem()', element,
                            sessionStorage
                            .getItem('inbox_' + $(element).attr('id')));
                    });
        ...
        $(document).on("click", "div#link-listAdditionalLinkDocs", function() {
            ...
            var table_listAdditionalLinkDocsSP = $('#table-listAdditionalLinkDocsSP').dataTable({
                dom: "<'row'<'col-sm-5'><'col-sm-4'><'col-sm-3'>>" +
                    "<'row'<'col-sm-12't>>" + "<'row'<'col-sm-8'><'col-sm-4'>>",
                language: {
                    url: "<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR3; ?>/dt_russian-docSPLinks.json"
                },
                ajax: {
                    url: "<?php echo __ROOT . __SERVICENAME_MAILNEW . __MAIL_INCOMING_CURRENT_WORKPATH . __MAIL_RESTR3; ?>/process/mailbox-incoming-process-showLogAdditionalLinkDocsSP.php",
                    type: "POST",
                    data: {
                        koddocmail: koddocmail
                    }
        ...
        $('#mail-incoming-filters-block *').filter('input, select').each(function(index, element) {
            ...
            console.log('Incoming sessionStorage checkpoint [reload]', index, tagName, itemID,
                itemName,
