...
$_reqDB = mysqli_fetch_assoc(mysqlQuery("SELECT inbox_rowIDs_links FROM " . __MAIL_INCOMING_TABLENAME . " WHERE koddocmail='{$koddocmail}'"));
$rowIDS = $_reqDB['inbox_rowIDs_links'] != "" && $_reqDB['inbox_rowIDs_links'] != null ? $_reqDB['inbox_rowIDs_links'] : null;
#
#
/*
	...
	->on('preGet', function ($editor, $id) use ($rowIDS) {
		$editor->where(function ($q) use ($rowIDS) {
			if ($rowIDS != null && !empty($rowIDS)) {
				$q->where(__MAIL_INCOMING_TABLENAME . '.ID', '( SELECT ID FROM ' . __MAIL_INCOMING_TABLENAME . ' WHERE ID IN (' . $rowIDS . ') )', 'IN', false);
				$q->order(__MAIL_INCOMING_TABLENAME . '.inbox_docID desc');
			} else {
				$q->where(__MAIL_INCOMING_TABLENAME . '.ID', '-1');
			}
		});
	})
