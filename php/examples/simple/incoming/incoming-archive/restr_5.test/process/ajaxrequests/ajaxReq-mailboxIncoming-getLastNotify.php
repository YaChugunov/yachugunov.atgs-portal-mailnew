...
if (isset($_SESSION['password']) && isset($_SESSION['login'])) {
	if (checkUserAuthorization($_SESSION['login'], $_SESSION['password']) == -1) {
		...
		if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && $_koddocmail != "" && $_action != "") {
			$_reqDB = mysqli_fetch_array(mysqlQuery("SELECT * FROM " . __MAIL_INCOMING_PREFIX . "_logControl WHERE `action` = '{$_action}' AND `koddocmail` = '{$_koddocmail}' AND comment LIKE '%{$_parameter}%' ORDER BY `timestamp` DESC LIMIT 1"));
