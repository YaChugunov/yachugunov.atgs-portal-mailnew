...
if (isset($_SESSION['password']) && isset($_SESSION['login'])) {
	if (checkUserAuthorization($_SESSION['login'], $_SESSION['password']) == -1) {
		...
		if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && "" != $_koddocmail) {
			$sqlReqString1 = "SELECT ispolStatus FROM " . __MAIL_INCOMING_PREFIX . "_logCheckouts WHERE koddocmail = '{$_koddocmail}' AND userid = '{$_userid}' AND koddel <> 'deleted'";
			$_reqDB_1 = mysqli_fetch_array(mysqlQuery("SELECT ispolStatus FROM " . __MAIL_INCOMING_PREFIX . "_logCheckouts WHERE koddocmail = '{$_koddocmail}' AND userid = '{$_userid}' AND (koddel IS NULL OR koddel <> 'deleted')"));
			$_reqDB_2 = mysqli_fetch_array(mysqlQuery("SELECT inbox_docContractor_kodzayvispol, inbox_docContractorID FROM " . __MAIL_INCOMING_TABLENAME . " WHERE koddocmail = '{$_koddocmail}'"));
			$ispolStatus = $_reqDB_1['ispolStatus'];
			$ispolList1 = $_reqDB_2['inbox_docContractor_kodzayvispol'];
			$ispolList2 = $_reqDB_2['inbox_docContractorID'];
			if (isset($ispolStatus)) {
