...
function updateCntComments($db, $id, $koddocmail) {
	...
	$_reqDB_contrKOD = $db->sql("SELECT inbox_docContractor_kodzayvispol FROM " . __MAIL_INCOMING_TABLENAME . " WHERE koddocmail=" . $_koddocmail)->fetchAll();
	$_contrKOD = ($_reqDB_contrKOD[0]['inbox_docContractor_kodzayvispol'] != NULL && $_reqDB_contrKOD[0]['inbox_docContractor_kodzayvispol'] != "") ? $_reqDB_contrKOD[0]['inbox_docContractor_kodzayvispol'] : "";
	...
	$_reqDB_cntComments = mysqli_fetch_assoc(mysqlQuery("SELECT COUNT(*) as CommCounts FROM " . __MAIL_INCOMING_PREFIX . "_logComments WHERE action = 'COMM' AND koddocmail = '{$_koddocmail}'"));
...
function updateFields($db, $action, $id, $values, $row, $koddocmail) {
	...
	if ($action == "CRT") {
		...
		$msgComm .= "<p class='record-new comment'>" . $values[__MAIL_INCOMING_PREFIX . '_logComments']['commentText'] . "</p>";

		$db->update(__MAIL_INCOMING_PREFIX . '_logComments', array(
			'koddel'					=> '',
			'timestamp'				=> $_timestamp,
			'action'					=> $_action,
		...
		$db->insert(__MAIL_INCOMING_PREFIX . '_logChanges', array(
			...
			'changes'         => $_contrKOD,
			'changesOldVal'   => null,
			'changesNewVal'   => $values[__MAIL_INCOMING_PREFIX . '_logComments']['commentText'],
	...
	if ($action == "PREUPD") {
		$_REQ = mysqlQuery("UPDATE " . __MAIL_INCOMING_PREFIX . "_logComments SET prevcommentText = commentText WHERE id = {$id}");
	}
	#
	#
	if ($action == "UPD") {
		$_reqDB_3 = $db->sql("SELECT commentID FROM " . __MAIL_INCOMING_PREFIX . "_logComments WHERE id=" . $id)->fetchAll();
		...
		$msgComm .= "<span class='comment old'>" . $row[__MAIL_INCOMING_PREFIX . '_logComments']['prevcommentText'] . "</span>";
		$msgComm .= "<span class='comment arrows'>>>></span>";
		$msgComm .= "<span class='comment new'>" . $row[__MAIL_INCOMING_PREFIX . '_logComments']['commentText'] . "</span>";
		$msgComm .= "</p>";
		$db->update(__MAIL_INCOMING_PREFIX . '_logComments', array(
		...
		$db->insert(__MAIL_INCOMING_PREFIX . '_logChanges', array(
			...
			'newSettings'     => null,
			'changes'         => $_contrKOD,
			'changesOldVal'   => $row[__MAIL_INCOMING_PREFIX . '_logComments']['prevcommentText'],
			'changesNewVal'   => $row[__MAIL_INCOMING_PREFIX . '_logComments']['commentText'],
	...
	#
	if ($action == "DEL") {
		$_reqDB_3 = $db->sql("SELECT commentID, commentText FROM " . __MAIL_INCOMING_PREFIX . "_logComments WHERE id=" . $id)->fetchAll();
...
Editor::inst($db, __MAIL_INCOMING_PREFIX . '_logComments')
	->fields(
		Field::inst(__MAIL_INCOMING_PREFIX . '_logComments.id'),
		Field::inst(__MAIL_INCOMING_PREFIX . '_logComments.koddel'),
		Field::inst(__MAIL_INCOMING_PREFIX . '_logComments.timestamp')
			->getFormatter(Format::datetime(
				'Y-m-d H:i:s',
				'd.m.Y H:i:s'
			)),
		Field::inst(__MAIL_INCOMING_PREFIX . '_logComments.action'),
		Field::inst(__MAIL_INCOMING_PREFIX . '_logComments.koddocmail'),
		Field::inst(__MAIL_INCOMING_PREFIX . '_logComments.userid'),
		Field::inst(__MAIL_INCOMING_PREFIX . '_logComments.username'),
		Field::inst(__MAIL_INCOMING_PREFIX . '_logComments.commentID'),
		Field::inst(__MAIL_INCOMING_PREFIX . '_logComments.prevcommentText'),
		Field::inst(__MAIL_INCOMING_PREFIX . '_logComments.commentText'),
		Field::inst(__MAIL_INCOMING_PREFIX . '_logComments.commentAdd'),
		Field::inst(__MAIL_INCOMING_PREFIX . '_logComments.update_timestamp')
			->getFormatter(Format::datetime(
				'Y-m-d H:i:s',
				'd.m.Y H:i:s'
			)),
		Field::inst(__MAIL_INCOMING_PREFIX . '_logComments.update_userid'),
		Field::inst(__MAIL_INCOMING_PREFIX . '_logComments.update_username'),
	...
	->where(__MAIL_INCOMING_PREFIX . '_logComments.koddocmail', $koddocmail, '=')
	->where(__MAIL_INCOMING_PREFIX . '_logComments.koddel', 'deleted', '!=')
	->leftJoin('users', 'users.ID', '=', __MAIL_INCOMING_PREFIX . '_logComments.userid')
