		...
		if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && $_userid != "" && $_updateid != "") {
			...
			if (isset($_reqDB_Main) && !empty($_reqDB_Main['update_id'])) {
				...
				if ($_reqDB_Views && $_reqDB_Views['show_update'] == '1') {
					...
					if ($_action == 'check') {
						$_reqDB_Upd = mysqlQuery("UPDATE mailbox_updates_views SET show_progress='0' WHERE user_id='{$_userid}' AND update_id='{$_updateid}'");
						$output = "ok," . $progress;
					} elseif ($_action == 'mark') {
						$_reqDB_Upd = mysqlQuery("UPDATE mailbox_updates_views SET show_update='0' WHERE user_id='{$_userid}' AND update_id='{$_updateid}'");
						$output = "none";
					} else {
						$output = "ok," . $progress;
					}
