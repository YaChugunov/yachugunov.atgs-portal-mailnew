...
AND ( ((TIMESTAMPDIFF(SECOND, '$datetimenow', inbox_docDateDeadline) <= 0) AND inbox_controlIspolMailSpecialNotifyDL = '1') )
AND inbox_docContractorID != '' AND denyMailNotify != '1'
";
...
if (isset($_QRY_SELECT)) {
     while ($_ROW_SELECT = mysqli_fetch_assoc($_QRY_SELECT)) {
          ...
          $__notifyDL = $_ROW_SELECT['inbox_controlIspolMailSpecialNotifyDL'];
          //
          ...
          #
          # ОБНОВЛЕНИЕ 23.07.23 >>>>> >>>>> >>>>>
          # Делаем рассылку на базе рассылки для исполнителей, заменив в цикле рассылки массив исполнителей на массив руководителей
          # foreach ($arrispol as $value) > foreach ($arrCEO as $value)
          # Уведомление только для генерального директора
          # Бернер Л.И. (userid = 1123, kodispol = 245434747938111)
          # Рощин А.В. (userid = 1130, kodispol = 245434958791847)
          # 
          $arrCEO = [1123, 1130];
          # 
          # ОБНОВЛЕНИЕ 23.07.23 <<<<< <<<<< <<<<<
          ...
          if ("" != $__contrKOD && null != $__contrKOD) {
               foreach ($arrCEO as $value) {
                    # ОБНОВЛЕНИЕ 23.07.23 >>>>> >>>>> >>>>>
                    # Заменил выборку, сделал по ID а не под кщвшызщд
                    $__contractorDATA1 = $db_handle->runQuery("SELECT id, emailaddress, namezayvfio FROM mailbox_sp_users WHERE id=" . $value);
                    $__contractorDATA2 = $db_handle->runQuery("SELECT firstname, middlename, lastname FROM users WHERE id=" . $value);
                    # 
                    # ОБНОВЛЕНИЕ 23.07.23 <<<<< <<<<< <<<<<
                    ...
                    //
                    //
                    $_msgTitleStr = "Контроль исполнения (для руководства)";
                    switch ($_trigg) {
                         case 0:
                              $_triggStr = "";
                              $_triggTitleStr = "";
                    ...
                    #
                    # Тема сообения
                    $subjectTxt = "Почта АТГС [Входящие] : КИ (для руководства) : Документ № 1-2/" . $__docID . " : " . $_letterSubjText;
                    ...
                    if ($__contractorDATA1 && $__contractorDATA2) {
                         ...
                         if (is_writable($filename)) {
                              ...
                              if (!$mail->send()) {
                                   ...
                                   if ($_intDiffSeconds <= 0) {
                                        if (1 == $__notifyDL) {
                                             $_SQLReq_IncomingDoc_Update = "UPDATE mailbox_incoming SET inbox_controlIspolMailSpecialNotifyDL = 0 WHERE koddocmail = '{$__koddocmail}'";
                                             $_QRY_IncomingDoc_Update = mysqlQuery($_SQLReq_IncomingDoc_Update);
                                        }
                                   }
