...
$_SQLReq_Select1 = "
SELECT * FROM mailbox_outgoing
WHERE outbox_docDate BETWEEN '$datetime1' AND '$datetime2'
AND (outbox_controlIspolActive = '1' AND outbox_controlIspolCheckout <> '1')
AND ( ((TIMESTAMPDIFF(SECOND, '$datetimenow', outbox_docDateDeadline) <= " . REMINDER_1 . ") AND outbox_controlIspolMailReminder1 = '1') OR ((TIMESTAMPDIFF(SECOND, '$datetimenow', outbox_docDateDeadline) <= " . REMINDER_2 . ") AND outbox_controlIspolMailReminder2 = '1') OR ((TIMESTAMPDIFF(SECOND, '$datetimenow', outbox_docDateDeadline) <= 0) AND outbox_controlIspolMailNotifyDL = '1') )
AND outbox_docContractorID != ''
";
...
if (isset($_QRY_SELECT)) {
     while ($_ROW_SELECT = mysqli_fetch_assoc($_QRY_SELECT)) {
          //
          $__docID       = $_ROW_SELECT['outbox_docID'];
          $__docFileID   = $_ROW_SELECT['outbox_docFileID'];
          $__docFileIDadd = $_ROW_SELECT['outbox_docFileIDadd'];
          $__contrID     = $_ROW_SELECT['outbox_docContractorID'];
          $__contrKOD    = $_ROW_SELECT['outbox_docContractor_kodispolout'];
          $__deadline    = $_ROW_SELECT['outbox_docDateDeadline'];
          $__ispolMULTI  = $_ROW_SELECT['outbox_docContractorMULTI'];
          $__senderORG   = $_ROW_SELECT['outbox_docRecipient'];
          $__senderNAME  = $_ROW_SELECT['outbox_docRecipientName'];
          $__sourceID    = $_ROW_SELECT['outbox_docSourceID'];
          $__sourceDate  = date_create($_ROW_SELECT['outbox_docSourceDate']);
          $__docABOUT    = $_ROW_SELECT['outbox_docAbout'];
          $__docTYPE     = $_ROW_SELECT['outbox_docTypeSTR'];
          $__toSendEmail = $_ROW_SELECT['toSendEmail'];
          //
          $__koddocmail  = $_ROW_SELECT['koddocmail'];
          $__ispolIDs    = $_ROW_SELECT['outbox_docContractorID'];
          $__ispolNames  = $_ROW_SELECT['outbox_docContractorSTR'];
          $__ispolEmails = $_ROW_SELECT['outbox_docContractorEMAIL'];
          //
          $__controlActive = $_ROW_SELECT['outbox_controlIspolActive'];
          $__controlStatus = $_ROW_SELECT['outbox_controlIspolStatus'];
          //        
          $__reminder1 = $_ROW_SELECT['outbox_controlIspolMailReminder1'];
          $__reminder2 = $_ROW_SELECT['outbox_controlIspolMailReminder2'];
          $__notifyDL = $_ROW_SELECT['outbox_controlIspolMailNotifyDL'];
          //
          ...
          //
          $_docTitle = "Исходящий документ";
          //
          $_docDeadline = validateDate($_ROW_SELECT['outbox_docDateDeadline'], "Y-m-d") ? date('d.m.Y', strtotime($_ROW_SELECT['outbox_docDateDeadline'])) : "---";
          $_sourceDate = validateDate($_ROW_SELECT['outbox_docSourceDate'], "Y-m-d") ? date('d.m.Y', strtotime($_ROW_SELECT['outbox_docSourceDate'])) : "---";
          ...
          if ("" != $__contrKOD && null != $__contrKOD) {
               foreach ($arrispol as $value) {
                    ...
                    #
                    # Тема сообения
                    $subjectTxt = "Почта АТГС [Исходящие] : КИ : Документ № 1-1/" . $__docID . " : " . $_letterSubjText;
                    $subject    = "=?utf-8?B?" . base64_encode($subjectTxt) . "?=";
                    #
                    # ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
                    if ("" != $__docFileID) {
                         $__contractorDATA3 = $db_handle->runQuery("SELECT file_originalname, file_url FROM mailbox_outgoing_files WHERE id=" . $__docFileID);
                    ...
                    if ($__contractorDATA1 && $__contractorDATA2) {
                         ...
                         #
                         # Часть сообщения
                         $_msgText = '<span style="font-size:28px">' . $__IO . ', </span><br><span style="color: #E2574C">истекает срок исполнения</span> по исходящему документу №' . $__docID . ', по которому Вы являетесь ответственным';
                         #
                         #
                         # ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
                         #
                         ...
                         if (is_writable($filename)) {
                              ...
                              if (!$mail->send()) {
                                   ...
                                   fclose($handle);
                                   // Вставляем запись в log рассылок исходящей почты
                                   $_SQLReq_MailOutgoingLog_Insert = "INSERT INTO mailbox_outgoing_logControl (timestamp, action, koddocmail, controlTrigger, userid, userkod, usermail, description, sqlrequest, comment) VALUES ('{$datetimenow}', 'MAIL_REMIND', '{$__koddocmail}', '{$_trigg}', '{$__contractorID}', '{$value}', '{$email_to}', 'Напоминание о необходимости исполнения по исходящему документу', '{$sqlClear_select1}', 'Reminder1:{$__reminder1},Reminder2:{$__reminder2}')";
                                   $_QRY_MailOutgoingLog_Insert = mysqlQuery($_SQLReq_MailOutgoingLog_Insert);
                                   //
                                   //
                                   $_reqINS = mysqlQuery("INSERT INTO mailbox_outgoing_logChanges (timestamp, action, koddocmail, userid, kodispol, oldSettings, newSettings, changes, changesText, changesCount) VALUES ('{$datetimenow}', 'MAIL_REMIND', '{$__koddocmail}', '999999', '{$__contractorID}', null, null, '{$_trigg}', 'Ответственному ( <span class=text-success>{$__FIO}</span> ) отправлено автоматическое email-уведомление на <span class=text-success>{$email_to}</span> об истечении срока исполнения ({$_triggStr})', null)");
                                   //
                                   //
                                   //
                                   if ($_intDiffSeconds <= REMINDER_1) {
                                        if (1 == $__reminder1) {
                                             $_SQLReq_OutgoingDoc_Update = "UPDATE mailbox_outgoing SET outbox_controlIspolMailReminder1 = 0 WHERE koddocmail = '{$__koddocmail}'";
                                             $_QRY_OutgoingDoc_Update = mysqlQuery($_SQLReq_OutgoingDoc_Update);
                                        }
                                   }
                                   if ($_intDiffSeconds <= REMINDER_2) {
                                        if (1 == $__reminder2) {
                                             $_SQLReq_OutgoingDoc_Update = "UPDATE mailbox_outgoing SET outbox_controlIspolMailReminder2 = 0 WHERE koddocmail = '{$__koddocmail}'";
                                             $_QRY_OutgoingDoc_Update = mysqlQuery($_SQLReq_OutgoingDoc_Update);
                                        }
                                   }
                                   if ($_intDiffSeconds <= 0) {
                                        if (1 == $__notifyDL) {
                                             $_SQLReq_OutgoingDoc_Update = "UPDATE mailbox_outgoing SET outbox_controlIspolMailNotifyDL = 0 WHERE koddocmail = '{$__koddocmail}'";
                                             $_QRY_OutgoingDoc_Update = mysqlQuery($_SQLReq_OutgoingDoc_Update);
                                        }
                                   }
