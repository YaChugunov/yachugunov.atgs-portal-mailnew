...
if (isset($_QRY_SELECT)) {
    while ($_ROW_SELECT = mysqli_fetch_assoc($_QRY_SELECT)) {
        ...
        #
        # Тема сообения
        $subjectTxt = "Почта АТГС [Входящие] : Истекает срок исполнения по документу " . $__docID;
        ...
        if ("" != $__contrKOD && null != $__contrKOD) {
            foreach ($arrispol as $value) {
                ...
                //
                if ("" != $__docFileID) {
                    $__contractorDATA3 = $db_handle->runQuery("SELECT file_originalname, file_url FROM mailbox_incoming_files_test WHERE id=" . $__docFileID);
                    $__fileName        = (!empty($__contractorDATA3) && "" != $__contractorDATA3[0]['file_originalname']) ? "Основной прикрепленный файл" : "---";
