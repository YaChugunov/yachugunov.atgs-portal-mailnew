...
$_req1 = mysqlQuery("SELECT koddocmail FROM mailbox_incoming WHERE inbox_docDate BETWEEN '2021-01-01 00:00:00' AND '2023-12-31 23:59:59'");
while ($_req1Rows = mysqli_fetch_assoc($_req1)) {
	$koddocmail = $_req1Rows['koddocmail'];
	$_reqDB1 = mysqli_fetch_array(mysqlQuery("SELECT COUNT(*) as CommCounts FROM mailbox_incoming_logComments WHERE action IN ('COMM','FORM') AND koddocmail = '{$koddocmail}' AND koddel NOT IN ('deleted', 'NULL')"));
	$counts  = $_reqDB1['CommCounts'];
	$_reqDB2 = mysqlQuery("UPDATE mailbox_incoming SET cntComments = '{$counts}' WHERE koddocmail = '{$koddocmail}'");
