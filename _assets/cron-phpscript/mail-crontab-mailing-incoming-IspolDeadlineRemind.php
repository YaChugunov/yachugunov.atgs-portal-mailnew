...
AND ( ((TIMESTAMPDIFF(SECOND, '$datetimenow', inbox_docDateDeadline) <= " . REMINDER_1 . ") AND inbox_controlIspolMailReminder1 = '1') OR ((TIMESTAMPDIFF(SECOND, '$datetimenow', inbox_docDateDeadline) <= " . REMINDER_2 . ") AND inbox_controlIspolMailReminder2 = '1') OR ((TIMESTAMPDIFF(SECOND, '$datetimenow', inbox_docDateDeadline) <= 0) AND inbox_controlIspolMailNotifyDL = '1') )
     ...
     while ($_ROW_SELECT = mysqli_fetch_assoc($_QRY_SELECT)) {
          ...
          if ("" != $__contrKOD && null != $__contrKOD) {
               foreach ($arrispol as $value) {
                    ...
                    #
                    # Тема сообения
                    $subjectTxt = "Почта АТГС [Входящие] : КИ : Документ № 1-2/" . $__docID . " : " . $_letterSubjText;
